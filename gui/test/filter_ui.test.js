...
describe("SearchField interactions properly updates global state", () => {
  ...
  beforeEach(() => {
    ...
    sagaTester = new SagaTester({initialState: {filter: filterInitialState()},reducers});
  ...
  test("Number of elements in explorer match number of keys", () => {
    ...
    expect(parents.filterWhere(parent => parent.html() !== null)).toHaveProperty(
        "length",
        3
    );
    expect(parentElements.filterWhere(parentElement => parentElement.html() !== null)).toHaveProperty(
        "length",
        2
    );
  });
  test("Explorer displays key names", () => {
    let parents = wrapper.find(FilterHeader);
    let parentElements = wrapper.find(FilterParentElement);
    let parentText = parents.reduce((value,node,index)=>value += node.text(),"");
    expect(parentText).toContain("scanterm");
    expect(parentText).toContain("dkcclterm");
    expect(parentText).toContain("term");
    let parentElementText = parentElements.reduce((value,node,index)=>value += node.text(),"");
  ...
  test("ParentElements display data, without expansion needed", () => {
    ...
    expect(parentElementText).toContain("00759635/32!870970-basis:00759635-300751-katalog");
  });
  test("Only filtered elements are displayed, excluding ParentElement's", () => {
    // Setting up filter
    sagaTester.dispatch(filterActions.selectWhiteListPending({
      scanterm: {
        title: true
      }
    }));
  ...
  test("Clicking whitelist and apply applies white listing in global state and show appropriate elements", () => {
    ...
    let whitelistPendingButton = wrapper.find('.whitelist-button-header').first();
    whitelistPendingButton.simulate('click');
    // TODO make this more refactor agnostic
    let applyFilterButton = wrapper.find(BibliographicExplorer).find('button').first();
  ...
  test("Clearing filter by clicking should show all elements", () => {
    ...
    // TODO make this more refactor agnostic
    let clearFilterButton = wrapper.find(BibliographicExplorer).find('button').at(1);
