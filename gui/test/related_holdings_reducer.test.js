...
import relatedHoldingsReducer, {
  produceInitialState
} from "../app/reducers/related_holdings";
...
describe("Related holdings reducer unit tests", () => {
  ...
  test("Should handle pull successful", () => {
    ...
    expect(relatedHoldingsReducer(state, actions.pullSuccess(result))).toEqual(
      desiredState
    );
  });
  test("Should handle pull failed", () => {
    ...
    desiredState.loading = false;
    expect(
      relatedHoldingsReducer(state, actions.pullFailed(new Error(message)))
...
describe("Reducer negative testing, ignoring invalid actions", () => {
  ...
  test("Invalid input with pull failed", () => {
    ...
    expect(relatedHoldingsReducer(state, invalidAction2)).toEqual(
      desiredState2
    );
  });
