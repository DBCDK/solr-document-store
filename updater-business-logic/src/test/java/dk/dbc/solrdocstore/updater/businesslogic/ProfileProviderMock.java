 ...
 */
public class ProfileProviderMock implements ProfileProvider {

    private static final ObjectMapper O = new ObjectMapper();

    @Override
    public VipCoreProfile profileFor(String agencyId, String profile) {
        String path = "vipcore/profile/" + agencyId + "-" + profile + ".json";
        try (InputStream is = getClass().getClassLoader().getResourceAsStream(path)) {
            if (is == null)
                throw new IllegalStateException("Cannot find: " + path);
            ProfileServiceResponse response = O.readValue(is, ProfileServiceResponse.class);
            return new VipCoreProfile(response);
